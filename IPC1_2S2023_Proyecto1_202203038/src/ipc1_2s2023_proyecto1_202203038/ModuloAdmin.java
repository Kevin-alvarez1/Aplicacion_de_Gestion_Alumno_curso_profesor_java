...
public void contarYMostrarRepeticionesCursos(Vector<Vector<String>> vectorAlumnosAsignadosAsignados, JTable ListadoOficialCurso) {
    ...
    DefaultTableModel model = (DefaultTableModel) ListadoOficialCurso.getModel();

    // Obtener el número de filas en el modelo (suponemos que ya hay al menos tantas filas como cursos)
    int numRows = model.getRowCount();

    // Agregar los datos al modelo de la tabla en la columna 4
    for (int i = 0; i < numRows; i++) {
        String curso = (String) model.getValueAt(i, 1); // Obtener el nombre del curso de la columna 2
        Integer repeticiones = conteoCursos.get(curso); // Obtener el número de repeticiones del mapa

        // Asegurarse de que repeticiones no sea null
        if (repeticiones != null) {
            model.setValueAt(repeticiones + " ", i, 3); // Actualizar la columna 4 con el número de repeticiones
        }
    }
}
...
this.dispose();
   ...
   // TODO add your handling code here:
    ...
    llenarTablaCurso();
    contarYMostrarRepeticionesCursos(vectorAlumnosAsignadosAsignados,ListadoOficialCurso);
